...
using Vehicles.Interfaces;
    ...
    public class Engine
    {
        ...
        private IVehicle CreateTruck(string[] truckInfo)
        {
            var truckFuelQuantity = double.Parse(truckInfo[1]);
            var truckLiterPerKm = double.Parse(truckInfo[2]);

            return new Truck(truckFuelQuantity, truckLiterPerKm);
        }

        private IVehicle CreateCar(string[] carInfo)
        {
            var carFuelQuantity = double.Parse(carInfo[1]);
            var carLiterPerKm = double.Parse(carInfo[2]);

            return new Car(carFuelQuantity, carLiterPerKm);
        }
    }
}
